...
Address.prototype.toString = function() {
  let address = {}
  address.main = Base58Check.encode(this.toBuffer());
  if (this.network.name === 'bch' || this.network.name === 'bsv') {
    ...
    if (this.network.name === 'bch') {
      address.bch = `${this.network.prefix}:${base32.encode(payload)}`;
    } else if (this.network.name === 'bsv') {
      address.bsv = base32.encode(payload);
    }
  } else if (this.network.name === 'zec') {

  } else if (this.network.name === 'livenet' || this.network.name === 'dgb') {
      var prefix = this.network.bech32prefix || this.network.prefix;
      var version = 0; // Only supporting segwit v0 for now
      let addr = Bech32.encode(prefix, version, this.hashBuffer);
      if (this.network.name === 'btc') {
        address.segwit = addr;
      } else if (this.network.name === 'dgb') {
        address.dgb = addr;
      }
  }
  return address
...
function decodeCashAddress(address) {
  ...
  if (prefix) {
    ...
    var netNames = ['bch','testnet','regtest'];
    var i;
